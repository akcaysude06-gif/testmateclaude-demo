...
class JiraService:
    ...
    def create_issue(
        ...
        """Create a Jira Task tagged automation-library. Returns the new issue key and URL."""
        base = self._normalize_url(instance_url)
            ...
            "content": [
                {
                    "type": "heading",
                    "attrs": {"level": 3},
                    "content": [{"type": "text", "text": "Manual Test Description"}],
                },
                {
                    "type": "paragraph",
                    "content": [{"type": "text", "text": manual_description or "(no description provided)"}],
                },
                {
                    "type": "heading",
                    "attrs": {"level": 3},
                    "content": [{"type": "text", "text": "Generated Selenium Code"}],
                },
                {
                    "type": "codeBlock",
                    "attrs": {"language": "python"},
                    "content": [{"type": "text", "text": generated_code or ""}],
                },
                {
                    "type": "paragraph",
                    "content": [
        ...
        try:
            ...
            raise HTTPException(status_code=400, detail=f"Cannot reach Jira at {base}.")
        except requests.exceptions.Timeout:
            raise HTTPException(status_code=408, detail="Jira request timed out.")

        if not resp.ok:
            detail = self._jira_error_message(resp)
            raise HTTPException(status_code=resp.status_code, detail=f"Jira issue creation failed: {detail}")
    ...
    def get_issue_current_status(
        ...
        """Fetch current status name for a single issue. Returns 'To Do' on any error."""
        base = self._normalize_url(instance_url)
        url  = f"{base}/rest/api/3/issue/{issue_key}"
        try:
            resp = requests.get(
    ...
    def get_issues_by_label(
        ...
        api_token: str,
        project_key: str,
        label: str = "automation-library",
    ) -> List[Dict]:
        """Fetch all issues in project with a given label, ordered newest first."""
