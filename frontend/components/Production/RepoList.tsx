...
const langColor: Record<string, string> = {
    ...
    Python: 'bg-green-500',      Java: 'bg-red-500',
    'C++': 'bg-pink-500',        Go: 'bg-cyan-500',
    Rust: 'bg-orange-500',       Ruby: 'bg-red-600',
    PHP: 'bg-purple-500',
};
...
const RepositoryList: React.FC<RepositoryListProps> = ({
    repositories, selectedRepo, onSelectRepo, isLoading, error, onRetry,
}) => (
    <div className="bg-white/5 backdrop-blur-lg rounded-2xl border border-white/10 overflow-hidden h-[calc(100vh-300px)] flex flex-col">
        ...
        <div className="bg-gradient-to-r from-purple-600/20 to-pink-600/20 border-b border-white/10 p-4">
            ...
            {!isLoading && !error && (
                <p className="text-sm text-purple-300 mt-1">{repositories.length} repositories</p>
            )}
        </div>
            ...
            {isLoading ? (
                ...
                <div className="flex flex-col items-center justify-center h-full">
                    ...
                    <button onClick={onRetry}
                        className="bg-red-500/20 hover:bg-red-500/30 text-red-300 px-4 py-2 rounded-lg text-sm">
                        Try Again
                    </button>
                </div>
                ...
                <div className="flex flex-col items-center justify-center h-full text-center">
                    ...
                    <p className="text-purple-300 text-sm">No repositories found</p>
                </div>
            ) : (
                        ...
                        <button key={repo.id} onClick={() => onSelectRepo(repo)}
                            ...
                            <div className="flex items-start justify-between mb-2">
                                <div className="flex items-center space-x-2 flex-1 min-w-0">
                                    ...
                                    <h4 className="font-semibold text-white truncate text-sm">{repo.name}</h4>
                                </div>
                                {repo.private
                                    ? <Lock className="w-3 h-3 text-yellow-400 flex-shrink-0 ml-2" />
                                    : <Globe className="w-3 h-3 text-green-400 flex-shrink-0 ml-2" />}
                            </div>
                            <p className="text-xs text-purple-200 mb-2 line-clamp-2">
                                {repo.description || 'No description'}
                            </p>
                            <div className="flex items-center justify-between">
                                {repo.language && (
                                    <div className="flex items-center space-x-1">
                                        <div className={`w-2 h-2 rounded-full ${langColor[repo.language] || 'bg-gray-500'}`} />
                                        <span className="text-xs text-purple-300">{repo.language}</span>
                                    </div>
                                )}
                                <span className="text-xs text-purple-400">{timeAgo(repo.updated_at)}</span>
                            </div>
                        </button>
                    ))}
                </div>
